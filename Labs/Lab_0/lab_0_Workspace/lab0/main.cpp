...
static void setupButtons()
{
    ...
    btnRestart.begin();
    btnRestart.setTickIntervalMs(BUTTON_TICK_MS);
...
static void drawStopwatchScreen(tContext &context, uint32_t currentSec, bool running)
{
    ...
    uint32_t hours = 0;
    uint32_t minutes = 0;
    uint32_t seconds = currentSec;
    uint32_t milseconds = gStopwatchMs;

    char hoursSTR[20];
    //char minutesSTR[10];
    //char secondsSTR[10];
    //char milsecondsSTR[10];
    ...
    if (running){
        ...
        GrStringDrawCentered(&context, hoursSTR, -1, 64, 50, false);
    }
    else{
